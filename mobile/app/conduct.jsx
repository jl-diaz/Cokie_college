  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showAlert({
        type: 'error',
        title: t('dashboard.error', 'Error'),
        message: t('conduct.saveCodeError', 'No se pudo guardar el código.')
      });
    } finally {
      setSaving(false);
    }
      ...
      onConfirm: async () => {
        try {
          ...
          showAlert({
            ...
            message: t('conduct.codeDeletedSuccess', 'Código eliminado correctamente.')
          });
        } catch (error) {
          showAlert({
            type: 'error',
            ...
            >
              ...
              <View style={styles.formGroup}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  <Text style={[styles.label, { marginBottom: 0 }]}>{t('common.code', 'Código')}</Text>
                  {!editingCode && (
                    <View style={{ backgroundColor: '#e0f2fe', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 10 }}>
                      <Text style={{ fontSize: 11, fontWeight: '700', color: '#0284c7' }}>{t('common.autoGenerated', 'Generado Automático')}</Text>
                    </View>
                  )}
                </View>
                <TextInput
