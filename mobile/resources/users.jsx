...
export default function UsersScreen() {
  ...
  // ── RetoFecha: Estado para el DatePicker de fecha de nacimiento ──
  const [showBirthDatePicker, setShowBirthDatePicker] = useState(false);

  // Form states (incluye birth_date y materia_principal)
  ...
  const handleOpenEditModal = (user) => {
    ...
    setFormData({
      ...
      level: user.level || '',
      materia_principal: user.specialty_subject_id || user.materia_principal || '',
      birth_date: user.birth_date || ''
    });
    setModalVisible(true);
  };

  // ── RetoFecha: Función para calcular edad ──
  ...
  // ── RetoFecha: Función para validar edad según el rol ──
  const validateAge = (birthDate, role) => {
    ...
    if (role === 'student') {
      ...
      if (age < 18) {
        return { valid: false, message: `El usuario con rol ${role.replace('_', ' ')} debe ser mayor de 18 años. Edad calculada: ${age} años.` };
      }
    }
    // Para 'cafetin' no se aplica validación de edad
    
    return { valid: true, age };
  };

  // ── RetoFecha: Handler del DatePicker ──
  ...
  const handleSaveUser = async () => {
    ...
    // ── RetoFecha: Validación de edad según el rol ──
    ...
    try {
      if (editingUser) {
        // Edit Mode (incluye materia_principal y birth_date)
        ...
        // Create Mode (incluye materia_principal y birth_date)
        ...
        >
          <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
            ...
            <ScrollView style={styles.modalForm}>
              ...
              {/* ── RetoFecha: Campo de Fecha de Nacimiento con DatePicker ── */}
              <View style={styles.formGroup}>
...
const createStyles = (Colors) => StyleSheet.create({
  ...
  dropdownItem: { padding: 12, borderBottomWidth: 1, borderBottomColor: Colors.gray[100] },
